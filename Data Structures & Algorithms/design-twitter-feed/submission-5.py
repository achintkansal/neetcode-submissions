class Twitter:
    ...
    def postTweet(self, userId: int, tweetId: int) -> None:
        ...
        if userId in self.user_tweet:
            ...
            self.user_tweet[userId] = [[self.time, tweetId]]

        if not userId in self.user_follower:
            self.user_follower[userId] = set()
        print('h',self.user_tweet)

    def getNewsFeed(self, userId: int) -> List[int]:
        print('a',self.user_tweet)
        print('b', self.user_follower)
        res = []
        ...
        tweet_sort = sorted(tweets, reverse = True)
        
        count = 0
        for tweet in tweet_sort:
            if count == 10:
                break
            res.append(tweet[1])
            count += 1
